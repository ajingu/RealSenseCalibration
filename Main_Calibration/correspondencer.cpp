...
{
	vector<Point3d> Correspondencer::GetCornersInCameraWorld(Vec3d rvec, Vec3d tvec)
	{
		double half_side = MARKER_SIDE / 2;
	...
	Correspondencer::Correspondencer(const map<string, Mat>& camera_intrinsics_map, const map<string, Mat>& dist_coeffs_map)
		...
		for (int time_idx = 0; time_idx < TIMES; time_idx++)
		{
			for (int camera_idx = 0; camera_idx < CAMERAS; camera_idx++)
			{
				...
				for (int i = 0; i < ids.size(); i++)
				{
					...
					vector<Point3d> corners3D = GetCornersInCameraWorld(marker_transform.rvec, marker_transform.tvec);
	...
	void Correspondencer::CalculateTransforms(vector<vector<Point3d>>& object_points, vector<vector<Point2f>>& image_points, vector<Vec3d>& camera_rvecs, vector<Vec3d>& camera_tvecs)
	{
	...
	void Correspondencer::Write(vector<vector<vector<Observation>>>& observations, vector<Vec3d>& camera_rvecs, vector<Vec3d>& camera_tvecs, vector<Vec3d>& base_rvecs, vector<Vec3d>& base_tvecs, map<int, Transform>& marker_transforms_from_base)
	{
		ofstream fout;
